...
-ANSWER: In all three fires there are patches of more severe fire intesity, especially in the middle, and the edges all have lower intensity. In the holidy and beachie fires, the legend includes a very high number (127) that is not part of the fire severity scale but marks land outside the boundaries of the fires and makes the entire area around the fire a different color instead of leaving it empty space.
...
- ANSWER crs: 
- - Holiday: the projection is UTM in zone 10 with the datum NAD83
- - Beachie: the projection is aea with the datum NAD83
- - Terwilliger: the projection is aea with the datum NAD83
- ANSWER units: Holiday: meters, Beachie: meters, Terwilliger: meters
- ANSWER the same? : The units are meters for all of them, but the projection for the holiday fire raster is different from the beachie and terwilliger rasters which are identical. The datum is NAD83 for all of them.
...
- ANSWER resolution: Holiday: 20 by 20 meters, Beachie: 20 by 20 meters, Terwilliger: 30 by 30 meters
- ANSWER the same? : The Holiday and Beachie rasters are both a 20 by 20 resolution, but the Terwilliger raster is 30 by 30 resolution.
...
```
- ANSWER minmax: Holiday: 1 to 127 Beachie: 1 to 127 Terwilliger: 1 to 4
- ANSWER the same? :  the holiday and beachie rasters both go from 1 to 127, but the terwilliger raster goes from 1 to 4.
...
#remake data frames
holiday_rast_df<- as.data.frame(holiday_rast, xy=TRUE)
beachie_rast_df<- as.data.frame(beachie_rast, xy=TRUE)     #do this for later bar plot
terwilliger_rast_df<- as.data.frame(terwilliger_rast, xy=TRUE)

#re-convert continuous to discrete values for holiday and beachie
holiday_rast_df$Layer_1<- as.factor(holiday_rast_df$Layer_1)
beachie_rast_df$Layer_1<- as.factor(beachie_rast_df$Layer_1)  #do this for later bar plot


ggplot() +
  geom_raster(data = holiday_rast_df , aes(x = x, y = y, fill = Layer_1)) +
  #geom_raster(data = beachie_rast_df , aes(x = x, y = y, fill = Layer_1)) +     (taking too long to run)
  geom_raster(data = terwilliger_rast_df , aes(x = x, y = y, fill = SoilBurnSe)) +
  scale_fill_brewer(palette= "Spectral", direction= -1)
...
terwilliger_rast_df$SoilBurnSe<- as.character(terwilliger_rast_df$SoilBurnSe)

#convert categories to numbers
terwilliger_rast_df$SoilBurnSe[terwilliger_rast_df$SoilBurnSe=="High"]<- "4"
terwilliger_rast_df$SoilBurnSe[terwilliger_rast_df$SoilBurnSe=="Moderate"]<- "3"
terwilliger_rast_df$SoilBurnSe[terwilliger_rast_df$SoilBurnSe=="Low"]<- "2"
terwilliger_rast_df$SoilBurnSe[terwilliger_rast_df$SoilBurnSe=="Unburned"]<- "1"
unique(terwilliger_rast_df$SoilBurnSe)
...
crs(ecoregions,proj=TRUE)==crs(holiday_rast,proj=TRUE)
```
...
##holiday fire
ggplot() +
  geom_bar(data=holiday_rast_df,aes(x=Layer_1,y=(..count..)/sum(..count..), fill=as.factor(Layer_1))) +
  scale_fill_brewer(palette = "Spectral",direction=-1)
 
# ## beachie fire                  taking too long to run
# ggplot() +  
#   geom_bar(data=beachie_rast_df,aes(x=Layer_1,y=(..count..)/sum(..count..), fill=as.factor(Layer_1))) +
#   scale_fill_brewer(palette = "Spectral",direction=-1)
  
## terwilliger fire
ggplot() +
  geom_bar(data=terwilliger_rast_df,aes(x=SoilBurnSe,y=(..count..)/sum(..count..), fill=as.factor(SoilBurnSe))) +
  scale_fill_brewer(palette = "Spectral",direction=-1)
...
ANSWER: Low to moderate fire severity is the most common level of severity. In the holiday fire, most of the area experienced moderate fire severity while in the terwilliger fire, most of the area experienced low fire severity. Neither had a huge proportion of high fire severity but there was more in the holiday fire, which was a bigger fire all around. In the Haldofsky reading, they said it was easier to control fire severity than to control area burned in a year. This makes sense because foresters can reduce fuel loads in a forest to reduce fire severity but they can't manually lower temperatures to reduce the number of days with a high likehlihood of fire as climate change increases temperature. In the graphs, they shows that both fires had higher proportions of low to moderate fire rather than severe fire, even in the bigger fire, which could reflect the work that foresters have done to manage fuel loads, and is promising for future forest management in keeping most of our fire at low or moderate severity.
