...
```{r}
###  your code here
row.names(sp_occ)<- row_labs_sp
row.names(sp_mass)<- row_labs_sp
...
##probEat takes the size of a given predator, size of a given prey, and alpha, beta, and gamma values and outputs the probability that the given predator will eat the given prey
probEat<- function(pred_size, prey_size, a, B, Y) {       ##a=alpha, B=beta, Y=gamma
  prob<- inv_logit(a + B*log(prey_size/pred_size) + Y*log(prey_size/pred_size)^2)
  return(round(prob,2))
}

probEat(50,100, alpha, beta, gamma) ##test
```
...
food_interactions <- matrix(0,nrow=length(row_labs_sp),ncol=length(row_labs_sp))
colnames(food_interactions)<- rownames(sp_occ)
rownames(food_interactions)<- rownames(sp_occ)
#    or for (food_interactions[row,col])

#for x,y possible
for (x in row_labs_sp) {   #rows
  for (y in row_labs_sp){     #columns
    x2<- sp_mass[x,"f"]
    y2<- sp_mass[y,"f"]
    new_prob<- probEat(x2, y2, alpha, beta, gamma) #i is for each item
    food_interactions[x,y]<- new_prob
  }
}
```
...
foodweb<- sweep(food_interactions,MARGIN=2,STATS=carnivores, FUN="*")
```
...
foodweb_V2<- sweep(foodweb,MARGIN=1,STATS=sp_occ[,2],FUN="*")

sum(rowSums(foodweb_V2)>0)
```
...
all_time_periods<- list()
for (time in 1:23){
  web_per_period<- sweep(foodweb,MARGIN=1,STATS=sp_occ[,time],FUN="*")
  all_time_periods[[time]]<- web_per_period
}

all_time_periods[[5]][1:5,1:5]
```
...
web_graphs<- lapply(all_time_periods, graph_from_adjacency_matrix, mode="directed",weighted= TRUE,diag=FALSE)
web_graphs[[1]]
```
...
library(networkD3)
groups_1<- carnivores[V(web_graphs[[1]])$name] #V function returns vertices, give it a graph; returns a 1 column dataframe called name; 

###  convert to a network 3d object
igraph_1<- web_graphs[[1]]
time1_network3d<- igraph_to_networkD3(igraph_1, group=groups_1,
                                      what="both")

###  plot the network     #bipartite- expects linear flow, sankey may not be best
forceNetwork(Links = time1_network3d$links,   ##simpler than sankey
             Nodes = time1_network3d$nodes,
...
# your code here
#group by carnivores
groups_3<- carnivores[V(web_graphs[[3]])$name]

##convert to 3d network
igraph_3<- web_graphs[[3]]
time3_network3d<- igraph_to_networkD3(igraph_3, group=groups,
                                      what="both")
###  plot 
forceNetwork(Links = time3_network3d$links,   ##simpler than sankey, 
             Nodes = time3_network3d$nodes,
            Source = "source", Target = "target",
            Value = "value",  NodeID = "name",
             Group = "group", 
            opacity = 0.8, zoom=TRUE, opacityNoHover = 0.9)
...
# your code here
#group by carnivores
groups_5<- carnivores[V(web_graphs[[5]])$name]

##convert to 3d network
igraph_5<- web_graphs[[5]]
time5_network3d<- igraph_to_networkD3(igraph_5, group=groups,
                                      what="both")
###  plot 
forceNetwork(Links = time5_network3d$links,   ##simpler than sankey, 
             Nodes = time5_network3d$nodes,
            Source = "source", Target = "target",
            Value = "value",  NodeID = "name",
             Group = "group", 
            opacity = 0.8, zoom=TRUE, opacityNoHover = 0.9)
